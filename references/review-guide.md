...
Lead with findings/repairs and unresolved decisions. Include an evidence table and coverage: candidate count, reviewed, partial, blocked, pending, excluded files, pruned directories and traversal errors. Keep boundaries separate from file counts. Label full/partial and document-only/selectively corroborated scope. Provide actual repairs in align mode and proposals in audit mode. Save a detailed ledger temporarily or at a requested report location, not as an unsolicited permanent project index.
