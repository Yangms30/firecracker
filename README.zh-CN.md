...
Firecracker 还会检查文档组织方式：入口、标题、重复规则、主题链接和存放位置。简短的导航文档可以指向详细参考资料；模块说明可以放在代码旁，也可以放在链接清晰的集中式 docs 目录中。

**长文档不一定有问题。** 主题连贯的参考文档可以保留原有长度。只有混合职责、重复内容或导航障碍影响查找时，才建议拆分。结构建议与内容矛盾分别报告；获批的重组计划需说明原章节与目标文件的对应关系，并保留链接、语义和代理指令的适用范围。
